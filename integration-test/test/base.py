...
class TestBase:
    ...
    def assert_output(self, target_address, target_output):
        ...
        for utxo in utxos:
            output = utxo.output
            if (
                output.address == target_output.address
                and output.amount == target_output.amount
                and (
                    (output.datum is None and target_output.datum is None)
                    or (datum_hash(output.datum) == datum_hash(target_output.datum))
                )
                and output.datum_hash == target_output.datum_hash
            ):
                found = True
            else:
                logging.warning(utxo.output)
